...
//
// Delays are written to the first sub-block (block0) of the subfile in the MWAX V2 format,
// where each row (per signal path) consists of: 16-bit RF input (not populated, set to zero),
// 16-bit whole sample delay (set to zeros), 32-bit float start/middle/end delays for each
// source, one row per source, where each delay value represents the fractional sample period.
// The offset into block0 is set with a #define.

// The first set of npath rows is the delays for the correlation poining  centre, and
// subsequent sets are for each beam poining direction.  In this test program, we just
// support one beam pointing (the correlation pointing centre), so the second set is
// populated with 0.0s - which it will be by default when we allocate the block0 buffer.

// Build: gcc -o make_test_subfile_v2_delay_tables make_test_subfile_v2_delay_tables.c
...
#define DELAY_ROW_BYTES 16             // 2 bytes RF ID, 2 bytes whole sample delay, 4 floats for start/middle/end delays
...
#define BLOCK_OFFSET 3305472           // byte offset to the start of the delays within block0
...
int main(int argc, char *argv[])
{
  ...
  {
    fprintf(stderr, "ERROR: failed to allocate memory for subfile delay buffer\n");
    exit(EXIT_FAILURE);
  }
  ...
  float *delays_start = (float *)(subfile_buffer + BLOCK_OFFSET);
  int delay_row_stride = DELAY_ROW_BYTES / sizeof(float);   // equivalent number of floats in a delay row
  for (int i = 0; i < num_sources; i++)
  {
    float delay;
    ...
    for (int k = 1; k < 4; k++)   // float 0 - ignore (it's actually two 16-bit ints), float 1 = start, float 2 = middle, float 3 = end
    {
